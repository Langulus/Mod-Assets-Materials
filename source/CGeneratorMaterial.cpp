...
/// Add an input/uniform to the material													
///	@param trait - the input to add														
///	@param allowDuplicates - whether duplication of traits is allowed			
///	@return the symbol																		
GLSL CGeneratorMaterial::AddInput(RRate rate, const Trait& traitOriginal, bool allowDuplicates) {
	...
	if (rate == RRate::PerAuto)
		rate = MaterialNode::DefaultTraitRate(traitOriginal.GetTraitMeta());
	...
	if (trait.IsUntyped())
		trait.SetDataID(MaterialNode::DefaultTraitType(trait.GetTraitMeta()), false);
	...
	PC_VERBOSE_MATERIAL("Added input " << ccCyan << trait << " as " << symbol << " @ " << rate);
	return symbol;
}

/// Add an output to the material															
///	@param rate - the rate at which the output is set								
///	@param trait - the output to add														
///	@param allowDuplicates - whether duplication of traits is allowed			
///	@return the symbol																		
GLSL CGeneratorMaterial::AddOutput(RRate rate, const Trait& traitOriginal, bool allowDuplicates) {
	if (!rate.IsShaderStage()) {
		throw Except::Content(pcLogSelfError
	...
	PC_VERBOSE_MATERIAL("Added output " << ccCyan << trait << " as " << symbol << " @ " << rate);
	return symbol;
}
...
const GLSL& CGeneratorMaterial::GetStage(ShaderStage::Enum stage) const {
	return GetDataAs<Traits::Code, GLSL*>()[stage];
}

/// Get a GLSL stage																				
///	@param stage - the stage index														
///	@return the code associated with the stage										
GLSL& CGeneratorMaterial::GetStage(ShaderStage::Enum stage) {
	return GetDataAs<Traits::Code, GLSL*>()[stage];
}

/// Get the list of GLSL stages																
///	@return the list																			
TAny<GLSL>& CGeneratorMaterial::GetStages() {
	return pcReinterpret<TAny<GLSL>&>(*static_cast<Any*>(GetData<Traits::Code>()));
}
...
void CGeneratorMaterial::Commit(ShaderStage::Enum stage, ShaderToken::Enum place, const GLSL& addition) {
	if (stage == ShaderStage::Counter)
		throw Except::Content(pcLogSelfError << "Unsupported stage");

	auto& code = GetStage(stage);
	if (code.IsEmpty()) {
		code += GLSL::From(stage);
		pcLogSelfVerbose << "Added default template for " << ShaderStage::Names[stage];
	}

	code.Select(place) >> addition;
	pcLogSelfVerbose << "Added code: ";
	pcLogSelfVerbose << addition.Pretty();
}
